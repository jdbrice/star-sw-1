...
void StFstFastSimMaker::FillSilicon(StEvent *event) {
	...
	for (int i = 0; i < nfsihit; i++) {
		...
		if (rnd_save > mInEff || mGEANTPassthrough){
			...
			// STEP 1 of proposal_StFstSimMaker.txt: give the hit its strip
			// indices. Without them the StHit-style constructor above leaves
			...
			// themselves, by up to the full 30 deg at the wedge edge.
			{
				...
				if (rStripMC >= 0 && mIdx > 0){
					...
					if (rStripMC < kFstNumRStripsPerWedge/2){
						...
						// signs per StFstHitMaker.cxx:159,163 AFTER the gap-sign
						// correction -- keep in sync with that file, or MC and
						// data will disagree by one full kFstStripGapPhi.
