...
StFttQAMaker::MakeRawHitQA(){
    ...
    for ( auto rawHit : mFttCollection->rawHits() ) {
        ...
        mFttData.stripcenter[mFttData.N]     = rawHit->stripCenter();
...
void StFttQAMaker::MakeClusterQA(){
    LOG_INFO << "MakeClusterQA :: Event " << GetIventNumber() << " has " << mFttCollection->clusters().size() << " clusters" << endm;
    ...
    for ( auto clu : mFttCollection->clusters() ) {
        ...
        mFttData.cMaxAdc_Pos[mFttData.cN]    = clu->maxStripCenter();
        mFttData.cMaxADC_LEdge[mFttData.cN]  = clu->maxStripLeftEdge();
        mFttData.cMaxADC_REdge[mFttData.cN]  = clu->maxStripRightEdge();
        // mH1d[ "U_MaxSc_Pos_diff" ]->Fill(clu->x()+clu->SC_MaxStrip());
        mH1d[ "U_MaxSc_Pos_diff" ]->Fill(clu->x()-clu->maxStripCenter());
...
void StFttQAMaker::MakePointQA(){
    ...
    for ( auto p : mFttCollection->points() ) {
        ...
        mFttData.pDiag1[mFttData.pN] = p->d1();
        mFttData.pDiag2[mFttData.pN] = p->d2();
        ...
        p->print();
        mH1d[ "XY_U_diff" ]->Fill( (p->x()+p->y())-(p->d1()*sqrt(2)) );
        ...
        for ( auto clu : mFttCollection->clusters() )
        {
            if( clu->plane() == p->plane() && clu->quadrant() == p->quadrant() && (clu->orientation() == 2 || clu->orientation() ==3 ))
            {
                ...
                diff = (p->x()+p->y())- (clu->maxStripCenter()*sqrt(2));
