...
Int_t StFttFastSimMaker::Make() {
    ...
    for (int i = 0; i < nstg; i++) {
        ...
        int quadrant_id = (volume_id - 1) % 100 / 10;
        ...
        if(quadrant_id == 2) x_local += 6.5;
        if(quadrant_id == 3) x_local -= 6.5;

        //misalignment
        if(quadrant_id == mMisQuad && plane_id == mMisDisk) 
        {
          x_local -= mDeltaU;
          y_local -= mDeltaV; 
          ...
          x_local =  xtemp*TMath::Cos(angle) + ytemp*TMath::Sin(angle); 
          y_local = -xtemp*TMath::Sin(angle) + ytemp*TMath::Cos(angle); 
        }
        
        // smear local x and y
        x_local = x_local + gRandom->Gaus(0, sigXY); // 100 micron blur according to approx sTGC reso
        y_local = y_local + gRandom->Gaus(0, sigXY); // 100 micron blur according to approx sTGC reso

        double x_smeared, y_smeared;
        y_smeared = y_local + 5.9;
        x_smeared = x_local;
        if(quadrant_id == 2) x_smeared = x_local - 6.5;
        if(quadrant_id == 3) x_smeared = x_local + 6.5;
        
        StFttPoint *point = new StFttPoint();
        point->setPlane(plane_id);
        point->setQuadrant(quadrant_id); // TODO this could be improved, but it is not used in the current implementation
        StThreeVectorD xyz;
        xyz.set(x_smeared, y_smeared, z);
        point->setXYZ( xyz );
