...
public:
    ...
    UChar_t vertexIndex() const {
        // BBB: bits 7…3 (5 bits, 0-31) — expanded from 6 bits to make room for type=4
        return (mVtxIndex >> 3) & 0x1F;
    }
    UChar_t trackType() const {
        // BBB: bits 2…0 (3 bits, 0-7) — expanded from 2 bits to hold kBLCVertexConstrained=4
    ...
    bool isBLCVertexConstrainedTrack() const { return (trackType() == StFwdTrack::kBLCVertexConstrained); } // BBB
    ...
    void setVtxIndex( UChar_t vtxIndex ) { mVtxIndex = pack5and3( vtxIndex, trackType() ); } // BBB
    void setTrackType( UChar_t trackType ) { mVtxIndex = pack5and3( vertexIndex(), trackType ); } // BBB
    void setVtxIndexAndTrackType( UChar_t vtxIndex, UChar_t trackType ) { mVtxIndex = pack5and3( vtxIndex, trackType ); } // BBB
    ...
    // BBB: kBLCVertexConstrained added; pack expanded to 5+3 bits to hold values 0-7
    ...
    static unsigned char inline pack5and3(unsigned int A, unsigned int B) {
        // BBB: 5 bits for vtxIndex (0-31), 3 bits for trackType (0-7)
