...
class StFwdTrackMaker : public StMaker {
  ...
  protected:
    ...
    // so I have removed them
        ...
        enum FwdVertexSource { kFwdVertexSourceUnknown, kFwdVertexSourceNone, kFwdVertexSourceTpc, kFwdVertexSourceMc, kFwdVertexSourceVpd, kFwdVertexSourceBLC }; // BBB: kFwdVertexSourceBLC = vertex from BLC track DCA-z fit
    ...
    */
    ...
    /** @brief Enable the per-wedge FST phi correction applied at MuDst-load time
     * (FstWedgeAligner in StFwdHitLoader.h) -- see bugreport_StFstHitMaker.txt.
     * Off by default.
    */
    ...
    */
    ...
    /** @brief Use measured beamline from DB for BLC track and vertex fitting.
     * When enabled, StFcsDb beamline parameters (x0,y0,dxdz,dydz) are passed
     * to the tracker each event. Default false (MC uses x=y=0 line).
    */
