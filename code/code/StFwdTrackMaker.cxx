...
int StFwdTrackMaker::Make() {
    ...
    /**********************************************************************/
    // Extract FCS ECAL clusters for FCSTRK (trkType=5) fitting
    {
        ...
        LOG_INFO << "FCSTRK: loaded " << fcsClusters.size() << " ECAL clusters for trkType=5 fitting" << endm;
...
StFwdTrack * StFwdTrackMaker::makeStFwdTrack( GenfitTrackResult &gtr, size_t indexTrack ){
    ...
    fwdTrack->setVtxIndexAndTrackType( gtr.mVertexIndex, gtr.mTrackType ); // BBB: replaced setVtxIndex(0/UCHAR_MAX); now stores trackType (incl. kBLCVertexConstrained=4) via pack5and3
    ...
    for ( auto zp : detectorZPlanes ){
        ...
        if ( detIndex != kFcsHcalId && detIndex != kFcsWcalId ){
            ...
            //AAA fix: project to showermax depth so track position matches getStarXYZfromColumnRow
            //         default (FcsZ=showerMaxZ).  Front-face projection (zdepth=0) caused ~2% slope
            //         in (FcsX, dx=FcsX-trkX) because cluster z=front+15cm > track z=front.
            float shwDepth = mFcsDb->getShowerMaxZ(det);
            StThreeVectorD xyzoff = mFcsDb->getDetectorOffset(det, shwDepth);
            StThreeVectorD planenormal = mFcsDb->getNormal(det);
            // DB position table not populated (e.g. in simulation) — fall back to hardcoded geometry
            if ( std::abs(xyzoff.z()) < 1.0 ) {
                //AAA: hardcoded fallback also needs showermax offset
                double shwZ_cos = shwDepth * cos(mFcsDb->getDetectorAngle(det)*M_PI/180.0);
                double shwZ_sin = shwDepth * sin(mFcsDb->getDetectorAngle(det)*M_PI/180.0);
                static const double ecalX[2] = { -17.399, 17.399 }; // North, South
                static const double hcalX[2] = { -21.285, 21.285 };
                int side = det % 2;
                double signAngle = (det%2==0) ? -1.0 : 1.0; // North negative, South positive
                if ( detIndex == kFcsWcalId )
                    xyzoff = StThreeVectorD( ecalX[side] + signAngle*shwZ_sin, -5.26, 710.16 + shwZ_cos );
                else
                    xyzoff = StThreeVectorD( hcalX[side] + signAngle*shwZ_sin,  1.80, 782.63 + shwZ_cos );
                LOG_WARN << "FCS getDetectorOffset returned near-zero for det=" << det
                         << ", using hardcoded geometry+showermax: " << xyzoff << endm;
            }
...
void StFwdTrackMaker::FillEvent() {
    ...
    // Add BLC vertex to StEvent (trkType=4 beam-line-constrained vertex)
    if ( mForwardTracker->getBLCVtxNTracks() > 0 ) {
        ...
        float cov[6] = { 0.01f, 0.f, 0.01f, 0.f, 0.f, sigZ*sigZ };  // sigmaXY=0.1cm
        bpv->setCovariantMatrix( cov );
    ...
    <TrackFinder nIterations="1">
        <Iteration nPhiSlices="1" > <!-- Options for first iteration -->
            <SegmentBuilder>
                ...
                <!-- AAA fix: Crit2_DeltaRho: require adjacent-disk r-strip change to be
                     physical. deltaRho = rhoParent - rhoChild (parent=outer disk, child=inner).
            ...
            <ThreeHitSegments>
                <!-- AAA fix: Crit3_3DAngle parameters are in DEGREES (not radians).
                     max=1 means 1 degree, not 1 radian. For a seed with r-segments (s,s,s+1),
                     seg1 points azimuthally (same-r disks) and seg2 jumps radially by 2.875 cm
                     over 7.5 cm z → 3D kink angle ~21 deg, far above 1 deg.
                     This cut incorrectly rejects valid seeds at eta~2.5-3 where one r-strip
                     change between disks is geometrically expected.
                     Replaced by Crit2_DeltaRho in SegmentBuilder. [Barak 2026-06-22] -->
                <!-- <Criteria name="Crit3_3DAngle" min="0" max="1" /> -->
                <!-- AAA fix: Crit3_2DAngle is also in degrees. For (s,s,s+1) seeds:
                     seg1_xy is azimuthal (both disk0,disk1 at same r-strip center) and
                     seg2_xy is radial (disk2 jumps to r-strip s+1, delta-r=2.875 cm).
                     The 2D angle between azimuthal and radial is ~90 deg >> 1 deg cut.
                     Same root cause as Crit3_3DAngle; removed for the same reason. -->
