...
public:
    // BBB: kBLCVertexConstrained added; bit layout expanded to 5+3 (matches StFwdTrack)
    ...
    UChar_t vertexIndex() const {
        // BBB: bits 7…3 (5 bits) — expanded from 6 bits
        return (mVtxIndex >> 3) & 0x1F;
    }
    UChar_t trackType() const {
        // BBB: bits 2…0 (3 bits) — expanded from 2 bits to hold kBLCVertexConstrained=4
    ...
    bool isBLCVertexConstrainedTrack() const { return (trackType() == StPicoFwdTrack::kBLCVertexConstrained); } // BBB
