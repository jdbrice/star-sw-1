...
// Track types: 0=Global 1=Beamline 2=Primary 3=FwdVtx 4=BLCVtx 5=FCSTRK
...
TH1F *hBunchId;
TH2F *hXY[2];              // 0=Ecal cluster xy, 1=Hcal cluster xy (type-independent)
TH1F *hE[2];               // 0=Ecal cluster E,  1=Hcal cluster E  (type-independent)
TH1F *hET[2];              // 0=Ecal cluster ET, 1=Hcal cluster ET (type-independent)
TH2F *hXYTrk[mNType][2];   // per-type track projection xy: 0=Ecal proj, 1=Hcal proj
TH1F *hETrk[mNType];       // per-type track |p|
TH1F *hPtTrk[mNType];      // per-type track pT
TH1F *hdx[mNType][2][2][3], *hdy[mNType][2][2][3], *hdp[mNType][2][2][3], *hdr[mNType][2][2][3];
TH2F *hxdx[mNType][2][3], *hydx[mNType][2][3], *hpdx[mNType][2][3], *hrdx[mNType][2][3];
TH2F *hxdy[mNType][2][3], *hydy[mNType][2][3], *hpdy[mNType][2][3], *hrdy[mNType][2][3];
TH2F *hxdp[mNType][2][3], *hydp[mNType][2][3], *hpdp[mNType][2][3], *hrdp[mNType][2][3];
TH2F *hxdr[mNType][2][3], *hydr[mNType][2][3], *hpdr[mNType][2][3], *hrdr[mNType][2][3];

// ECAL position resolution vs cluster energy (electron MC, Primary track projection)
// dx = fcsX - track_ecalProjection.X(); use Primary-type slice so sigma_track << sigma_pos
TH2F *hFcsEdxVsE;
TH2F *hFcsEdyVsE;
...
static int ntrk[mNType][2]={{0}};
static double tx[mNType][2][2][mMaxTrack],ty[mNType][2][2][mMaxTrack];
...
void InitMatch(int run, int set=-1) {
  ...
  // Matching histograms booked per track type (all types filled in one pass)
  for(int tt=0; tt<mNType; tt++){
    hXYTrk[tt][0] = new TH2F(Form("xyETrk%s",TTYPE[tt]),Form("Trk Ecal Projection %s; x; y",TTYPE[tt]),100,-150,150,100,-100,100);
    hXYTrk[tt][1] = new TH2F(Form("xyHTrk%s",TTYPE[tt]),Form("Trk Hcal Projection %s; x; y",TTYPE[tt]),100,-150,150,100,-100,100);
    hETrk[tt]  = new TH1F(Form("ETrk%s",TTYPE[tt]), Form("E Trk %s; p[GeV]", TTYPE[tt]),100,0.0,40.0);
    hPtTrk[tt] = new TH1F(Form("PTTrk%s",TTYPE[tt]),Form("PT Trk %s; pT[GeV]",TTYPE[tt]),100,0.0,5.0);
    for(int eh=0; eh<2; eh++){
      for(int cut=0; cut<mNCut; cut++){
        hxdx[tt][eh][cut]=new TH2F(Form("xdx%s%s_%s",EH[eh],CUT[0][cut],TTYPE[tt]),Form("dX(fcsX) %s %s; fcsX[cm]; dx[cm]",      EH[eh],TTYPE[tt]),100,-150,150,100,-100,100);
        hydx[tt][eh][cut]=new TH2F(Form("ydx%s%s_%s",EH[eh],CUT[0][cut],TTYPE[tt]),Form("dX(fcsY) %s %s; fcsY[cm]; dx[cm]",      EH[eh],TTYPE[tt]),100,-100,100,100,-100,100);
        hpdx[tt][eh][cut]=new TH2F(Form("pdx%s%s_%s",EH[eh],CUT[0][cut],TTYPE[tt]),Form("dX(fcsR*fcsPhi) %s %s; fcsR*Phi[cd]; dx[cm]",EH[eh],TTYPE[tt]),100,-150,150,100,-100,100);
        hrdx[tt][eh][cut]=new TH2F(Form("rdx%s%s_%s",EH[eh],CUT[0][cut],TTYPE[tt]),Form("dX(fcsR) %s %s; fcsR[cm]; dx[cm]",      EH[eh],TTYPE[tt]),100,   0,180,100,-100,100);
        hxdy[tt][eh][cut]=new TH2F(Form("xdy%s%s_%s",EH[eh],CUT[1][cut],TTYPE[tt]),Form("dY(fcsX) %s %s; fcsX[cm]; dy[cm]",      EH[eh],TTYPE[tt]),100,-150,150,100,-100,100);
        hydy[tt][eh][cut]=new TH2F(Form("ydy%s%s_%s",EH[eh],CUT[1][cut],TTYPE[tt]),Form("dY(fcsY) %s %s; fcsY[cm]; dy[cm]",      EH[eh],TTYPE[tt]),100,-100,100,100,-100,100);
        hpdy[tt][eh][cut]=new TH2F(Form("pdy%s%s_%s",EH[eh],CUT[1][cut],TTYPE[tt]),Form("dY(fcsR*fcsPhi) %s %s; fcsR*Phi[cm]; dy[cm]",EH[eh],TTYPE[tt]),100,-150,150,100,-100,100);
        hrdy[tt][eh][cut]=new TH2F(Form("rdy%s%s_%s",EH[eh],CUT[1][cut],TTYPE[tt]),Form("dY(fcsR) %s %s; fcsR[cm]; dy[cm]",      EH[eh],TTYPE[tt]),100,   0,180,100,-100,100);
        hxdp[tt][eh][cut]=new TH2F(Form("xdp%s%s_%s",EH[eh],CUT[2][cut],TTYPE[tt]),Form("fcsR*dPhi(fcsX) %s %s; fcsX[cm]; fcsr*rdphi[cm]",     EH[eh],TTYPE[tt]),100,-150,150,100,-100,100);
        hydp[tt][eh][cut]=new TH2F(Form("ydp%s%s_%s",EH[eh],CUT[2][cut],TTYPE[tt]),Form("fcsR*dPhi(fcsY) %s %s; fcsY[cm]; fcsr*dphi[cm]",      EH[eh],TTYPE[tt]),100,-100,100,100,-100,100);
        hpdp[tt][eh][cut]=new TH2F(Form("pdp%s%s_%s",EH[eh],CUT[2][cut],TTYPE[tt]),Form("fcsR*dPhi(fcsR*fcsPhi) %s %s; fcsR*Phi[cm]; fcsr*dphi[cm]",EH[eh],TTYPE[tt]),100,-150,150,100,-100,100);
        hrdp[tt][eh][cut]=new TH2F(Form("rdp%s%s_%s",EH[eh],CUT[2][cut],TTYPE[tt]),Form("fcsR*dPhi(fcsR) %s %s; fcsR[cm]; fcsr*dphi[cm]",      EH[eh],TTYPE[tt]),100,   0,180,100,-100,100);
        hxdr[tt][eh][cut]=new TH2F(Form("xdr%s%s_%s",EH[eh],CUT[3][cut],TTYPE[tt]),Form("dR(fcsX) %s %s; fcsX[cm]; dr[cm]",      EH[eh],TTYPE[tt]),100,-150,150,100,-100,100);
...
void RunMatch(StPicoDst *dst, StFcsDb* fcsDb, int iEvent){
  ...
  for(int tt=0; tt<mNType; tt++) ntrk[tt][same]=0;
  ...
  for(int it=0; it<nTracks; it++) {
    ...
    if(tt>=0 && tt<mNType &&
       ...
       fabs(x)<mHcalXmaxCut &&
       fabs(y)<mYmaxCut &&
       ntrk[tt][same]<mMaxTrack ){
      TVector3 p=t->momentum();
      hXYTrk[tt][0]->Fill(pe.X(),pe.Y());
      hXYTrk[tt][1]->Fill(ph.X(),ph.Y());
      hETrk[tt]->Fill(p.Mag());
      hPtTrk[tt]->Fill(p.Perp());
      tx[tt][same][0][ntrk[tt][same]]=pe.X();
      ty[tt][same][0][ntrk[tt][same]]=pe.Y();
      tx[tt][same][1][ntrk[tt][same]]=ph.X();
      ty[tt][same][1][ntrk[tt][same]]=ph.Y();
      ntrk[tt][same]++;
  ...
  for(int ic=0; ic<nClusters; ic++) {
    ...
    for(int i=0; i<2; i++){ //same event or mixed event
      ...
      for(int tt=0; tt<mNType; tt++){ //all track types filled in one pass
      for(int it=0; it<ntrk[tt][evt]; it++) {
	if(fcsx * tx[tt][evt][ehp][it] > 0){ //same side
	  //if(ehp==0 && fcse>0.5) continue;  //select MIP for ecal-trk
	  //if(ehp==1 && fcse<1.0) continue; //cut off low evenry for hcal-trk
	  double trkx=tx[tt][evt][ehp][it];
	  double trky=ty[tt][evt][ehp][it];
	  ...
	  if(fabs(dy)<mDCut) {
	    hdx[tt][ehp][nstb][i]->Fill(dx);
	    hxdx[tt][ehp][i]->Fill(fcsx,dx);
	    hydx[tt][ehp][i]->Fill(fcsy,dx);
	    hpdx[tt][ehp][i]->Fill(fcsrp,dx);
	    hrdx[tt][ehp][i]->Fill(fcsr,dx);
	    if(ehp==0 && i==0 && tt==2) hFcsEdxVsE->Fill(fcse, dx); // Primary track only (sigma_track << sigma_pos)
	  }
	  if(fcsy>0) {nstb=0;} else {nstb=1;}
	  if(fabs(dx)<mDCut){
	    hdy[tt][ehp][nstb][i]->Fill(dy);
	    hxdy[tt][ehp][i]->Fill(fcsx,dy);
	    hydy[tt][ehp][i]->Fill(fcsy,dy);
	    hpdy[tt][ehp][i]->Fill(fcsrp,dy);
	    hrdy[tt][ehp][i]->Fill(fcsr,dy);
	    if(ehp==0 && i==0 && tt==2) hFcsEdyVsE->Fill(fcse, dy); // Primary track only
	  }
	  if(fcsx>0) {nstb=0;} else {nstb=1;}
	  if(fabs(dr)<mDCut) {
	    hdp[tt][ehp][nstb][i]->Fill(rdp);
	    hxdp[tt][ehp][i]->Fill(fcsx,rdp);
	    hydp[tt][ehp][i]->Fill(fcsy,rdp);
	    hpdp[tt][ehp][i]->Fill(fcsrp,rdp);
	    hrdp[tt][ehp][i]->Fill(fcsr,rdp);
	  }
	  if(fcsr<70) {nstb=0;} else {nstb=1;}
	  if(fabs(rdp)<mDCut) {
	    hdr[tt][ehp][nstb][i]->Fill(dr);
	    hxdr[tt][ehp][i]->Fill(fcsx,dr);
	    hydr[tt][ehp][i]->Fill(fcsy,dr);
	    hpdr[tt][ehp][i]->Fill(fcsrp,dr);
	    hrdr[tt][ehp][i]->Fill(fcsr,dr);
	  }
	}//same side
      }//fwd track loop
      }//track type loop
