...
TString LHAPDF_DATA_PATH="/opt/star/sl73_x8664_gcc485/share/LHAPDF";
...
void Pythia6( TString mode="pp:DY", Int_t tune=370)
{
  ...
  pythia6->SetAttr("FRAME", "CMS");     // ensure attribute matches member
  pythia6->SetAttr("Ecms",  510.0);    // ensure DAttr("Ecms") != 0
  pythia6->SetBlue("proton");
  pythia6->SetAttr("BLUE",  "proton"); // ensure attribute matches member
  pythia6->SetYell("proton");
  pythia6->SetAttr("YELL",  "proton"); // ensure attribute matches member
  pythia6->PyTune(tune);
  ...
  // pythia6->Init() removed: primaryMaker->Init() calls it via StMaker::Init().
  // Calling Init() before AddGenerator causes double-Init; the second call runs
  // SetFrame("CMS", DAttr("Ecms")=0) triggering the mDirect==1.0 assertion.
...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
void runPythia( Int_t nevents=10, Int_t run=1, char* particle="JPsi", float vz=0.0, Int_t tune=370, bool fieldOff=false){
  cout<<"Random seed : "<<run<<endl;

  gROOT->ProcessLine(".L bfc.C");{
    TString simple = "y2022 geant gstar agml usexgeom ";
    if (fieldOff) simple += "FieldOff ";  // BFC MagF tag: zero field for GEANT transport
    bfc(0, simple );
  }
