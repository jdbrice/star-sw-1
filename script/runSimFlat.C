...
void trig( Int_t n=1, char* pid="ele", float e=0.0, float pt=1.5, int npart=1, int print=0){
	...
	if(e>0.0){
	    ...
	    //kinematics->Kine(npart, PID, e-0.01, e+0.01, 2.5,   4.00,, pi);	    
	    //kinematics->Kine(npart, PID, e-0.01, e+0.01, 2.5,  4.0, 0.0, 2*pi);	    
	    kinematics->Kine(npart, PID, 1, e, 2.5,  4.0, 0.0, 2*pi);	    
...
void runSimFlat( Int_t nevents=1000, Int_t run=1,
		 ...
		 int ecal=1, int print=0, bool fieldOff=false){
  gROOT->ProcessLine(".L bfc.C");{
    TString simple = "sdt20211016 y2024 geant gstar agml usexgeom";
