...
void loadLibs();
//void fwd_afterburner(const Char_t * fileList = "st_physics_23037002_raw_1000064.MuDst.root",size_t nEvents = 100 ){
//void fwd_afterburner(const Char_t * fileList = "root://xrdstar.rcf.bnl.gov:1095//home/starlib/home/starreco/reco/production_pp500_2022/ReversedFullField/P24ia/2022/003/23003024/st_fwd_23003024_raw_3000016.MuDst.root",size_t nEvents = 100 ){
void fwd_afterburner(const Char_t * fileList = "root://xrdstar.rcf.bnl.gov:1095//home/starlib/home/starreco/reco/production_pp500_2022/ReversedFullField/P24ia/2022/108/23108014/st_fwd_23108014_raw_2000026.MuDst.root",size_t nEvents = 100, int debug=0){
	...
	StFwdResidualMaker *fwdResidual = NULL;
	if (runFwdChain){
		// FwdTrackMaker
		...
		if (runFwdResidual && runFwdChain){
			// Per-plane FST/FTT alignment residuals -- runs on the in-memory MuDst
			// filled by StFwdTrackMaker this event, so it works even when MuDst
			// itself is never written to disk (real production: MuDst -> picoDst only).
			TString residualName( gSystem->BaseName(inMuDstFile) );
			residualName.ReplaceAll(".MuDst.root", ".FwdDetResidual.root");
			fwdResidual = new StFwdResidualMaker(residualName);
		}
	...
	if (fwdResidual) fwdResidual->Finish();
