...
Int_t StFstSlowSimMaker::InitRun(Int_t runnumber)
{
   ...
   // Go through GetObject(). (Same trap as script/dumpFstMapping.C.)
 ...
 *
 * WARNING, and it is the reason this maker exists: the outer-sensor gap term
 * below has to track StFstHitMaker.cxx:159,163. Those two lines are believed to
 * carry the wrong sign (fstgap/index.html, validated four ways on ~58M rows).
 * This inverts the code AS IT IS TODAY, so that MC and data agree. If the gap
 * fix lands in StFstHitMaker, THIS MUST BE FLIPPED TOO or MC and data will
 * disagree by one full kFstStripGapPhi. Having to say that is exactly the
 * duplication argument for doing the strip lookup from TGeo instead.
 */
