 ...
 * See FstSlowSim/index.html (supersedes proposal_StFstSimRawHitMaker.txt).
 *
 * Chain position:
 ...
 *
 * SCOPE OF THIS VERSION (v0, minimal -- see FstSlowSim/index.html):
 ...
 *    (FstSlowSim/fstclus/index.html), so cluster-size distributions will NOT match until
 ...
 * come from TGeo (find the FTUS node, MasterToLocal, divide by pitch), the way
 * script/checkFstWedgeGeom.C does -- that is the next step, hooked here as
 * setStripSource(kStripFromTGeo) but not yet implemented.
 */
