...
void plotMatch(char* data="202604",int run=0, int set=0){
  ...
  if(hasTruth){
    ...
    // ── Dedicated BLCVtx vs FCSCon momentum resolution page ────────────────
    // Double Gaussian fit: narrow core (sigma1) + wide tail (sigma2), shared center.
    // Core sigma1 is the resolution for well-reconstructed tracks.
    TH1F *hDqBLC = hTrkDqPt[4], *hDqFCS = hTrkDqPt[5];
    if(hDqBLC && hDqBLC->GetEntries()>0 && hDqFCS && hDqFCS->GetEntries()>0){
      auto fitDoubleGaus = [](TH1F *h, int col, double &sig1, double &sig2, double &frac1) {
      ...
      hBnorm->SetTitle("q/p_{T} residual: BLCVtx vs FCSCon;q/p_{T}^{reco}-q/p_{T}^{true} [(GeV/c)^{-1}];Norm. entries");
      hBnorm->Draw("hist"); hFnorm->Draw("hist same");
      double s1B,s2B,f1B, s1F,s2F,f1F;
      fitDoubleGaus(hBnorm, kBlue, s1B, s2B, f1B);
      fitDoubleGaus(hFnorm, kRed,  s1F, s2F, f1F);
      ...
      lMom->AddEntry(hFnorm,Form("FCSCon  N=%.0f", hDqFCS->GetEntries()),"l");
      ...
      printf("  FCSCon: sigma_core=%.3f  sigma_tail=%.3f  improvement=%.1f%%\n",s1F,s2F,improv);
    }
  }
