...
// Accessors and options
  ...
  public:
    ...
    void setupGenfitKalmanFitter(){
        ...
        // Used to refine an already-converged track with tight FST phi sigma (pitch/sqrt12)
    ...
    */
    ...
    /**
     * @brief Warm-start refinement of mFitTrack using tight FST phi sigma (pitch/sqrt12).
     *  Steps:
     *   1. Re-seed mFitTrack with current fitted momentum (warm start).
     *   2. Tighten FST V-measurement (phi) covariance in-place by factor 12.
     *   3. Run mWarmFitter (KalmanFitter, blowUpFactor=1e6) on mFitTrack.
     *  mFitTrack is left in the tight-phi fitted state on success; caller calls
     ...
     */
    bool warmFitFstTightPhi( Seed_t &seed ) {
        ...
        // Step 2: tighten FST V-measurement (phi direction) covariance in-place.
        // The 2×2 local plane covariance is stored as rawHitCov_ in each PlanarMeasurement.
        // V is the azimuthal direction; C_VV = (R × dphi_tight)²  ← what we want.
        // We scale C_VV by (pitch_full/pitch_sqrt12)² = 12; C_UU (radial) unchanged.
        const float scale = 12.f;  // (phi_full / phi_sqrt12)^2
        ...
        for (int ip = 0; ip < (int)mFitTrack->getNumPoints(); ip++) {
            ...
            for (int im = 0; im < (int)tp->getNumRawMeasurements(); im++) {
                ...
                // Tighten: C_VV /= 12  (V=azimuthal = curvature measurement)
                // C_UU (radial) unchanged — don't tighten r, only phi
        ...
        try {
            ...
            LOG_WARN << "warmFitFstTightPhi exception: " << e.what() << endm;
