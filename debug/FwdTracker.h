  ...
  public:
    ...
    */
    ...
    GenfitTrackResult refitTrack( GenfitTrackResult gtrGlobal ) {
        ...
        // Tight-phi warm refinement: if refit converged, run a second pass with
        // tight FST phi sigma (pitch/sqrt12).  warmFitFstTightPhi modifies mFitTrack
        // in-place and leaves it in the tight-phi state; refreshFromTrack() picks up
        // the updated momentum, charge, covariance, and chi2 for all track types.
        if ( gtrGlobalRefit.mIsFitConvergedFully ){
            bool warmOk = mTrackFitter->warmFitFstTightPhi( gtrGlobal.mSeed );
            if ( warmOk )
                gtrGlobalRefit.refreshFromTrack(); // BBB: propagate tight-phi state
        }
    ...
    std::vector<GenfitTrackResult> doBLCVertexFitting( std::vector<GenfitTrackResult> &beamlineTracks ) {
        ...
        double x_BL = mBeamlineHit.getX();
        double y_BL = mBeamlineHit.getY();
        ...
        for (auto &gtr : beamlineTracks) {
            ...
            GenfitTrackResult gtrVRefit = refitTrack(gtrV);
